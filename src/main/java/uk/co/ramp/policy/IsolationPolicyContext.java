...
public class IsolationPolicyContext {
    ...
    @Bean
    public IsolationPolicy isolationPolicy(DiseaseProperties diseaseProperties, DistributionSampler distributionSampler) {
        var singleCaseIsolationPolicy = new SingleCaseIsolationPolicy(isolationProperties(), distributionSampler);
    ...
    private IsolationProperties isolationProperties() {
        try (Reader reader = new FileReader(new File(ISOLATION_SETTINGS_LOCATION))) {
            ...
            String message = "An error occurred while parsing the isolation policy properties at " + ISOLATION_SETTINGS_LOCATION;
