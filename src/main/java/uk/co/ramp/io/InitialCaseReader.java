...
public class InitialCaseReader {
  ...
  @Autowired
  public InitialCaseReader(
      RandomDataGenerator generator, StandardProperties standardProperties, InputFiles inputFiles) {
  ...
  void tryPopulateCases(InputFiles inputFiles) {
    try (Reader reader = new FileReader(inputFiles.initialExposures())) {
      ...
      LOGGER.warn(
          "The set of initial cases is {} long, when {} was requested",
          cases.size(),
          standardProperties.initialExposures());
      fillOrTrimSet();
  ...
  void read(Reader reader) throws IOException {
    ...
    cases =
        list.stream()
            .map(ImmutableInitialCase::id)
            .filter(integer -> integer < population)
            .collect(Collectors.toSet());

    LOGGER.info(
        "After removing duplicates and limiting to values below the population size, {}, there are {} entries",
        population,
        cases.size());
  }
  ...
  void fillOrTrimSet() {
    ...
    if (set.size() > personLimit) {
      LOGGER.warn(
          "The list of initial infections contains {} cases, but the limit has been set to {}",
          set.size(),
          personLimit);
      ...
      LOGGER.warn(
          "The list initial infections contains {} entries, but the input has specified {}.",
          set.size(),
          personLimit);
      LOGGER.warn(
          "The additional {} infections will be randomly assigned.", personLimit - set.size());
