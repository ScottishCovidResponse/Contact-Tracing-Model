...
public class InfectionCreatorTest {
  ...
  public void setUp() {
    ...
    when(distributionSampler.uniformBetweenZeroAndOne())
        .thenAnswer(
            i -> distributionValues.get(counter.incrementAndGet() % distributionValues.size()));
  ...
  public void createRandomInfections() {
    ...
    InfectionCreator infectionCreator =
        new InfectionCreator(new Population(population), distributionSampler, initialCaseReader);
